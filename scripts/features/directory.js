...
function renderAccounts(accounts) {
  ...
  if (!list || !accounts.length) return;

  list.replaceChildren();
  accounts.forEach((account) => {
    const sideClass = account.side === 'groom' ? 'account-card-groom' : 'account-card-bride';
    const card = createElement('article', `account-card ${sideClass}`);
...
async function loadDirectory() {
  ...
  const [contactsResult, accountsResult] = await Promise.all([
    supabaseClient.from(CONTACTS_TABLE)
      .select('role_label, name, phone, display_order')
